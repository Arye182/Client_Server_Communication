...
string SearcherObjectAdapter::solutionConvertToString (vector<State<Point>*> solution_vector) {
  ...
  for (vector<State<Point>*>::reverse_iterator iterator_from_initial_state = solution_vector.rbegin();
       iterator_from_initial_state != solution_vector.rend(); ++iterator_from_initial_state ) {
    ...
    int prev_i =  (*iterator_from_initial_state)->getCameFrom()->getState().first;
    int prev_j =  (*iterator_from_initial_state)->getCameFrom()->getState().second;
    ...
    string_solution = string_solution + direction + "(" + to_string(cost) + ")"", ";
  }
  // delete last comma
