...
class AStar : public Searcher<T> {
 ...
 public:
  ...
  /**
   * this is a comparator that is responsible for priority queue.
   * a-star algorithm needs it to be by heuristic cost.
   */
  class myComparator {
   public:
  ...
  /**
   * this is the algorithm itself. it is basically like the BestFS but with
   * slightly changes (heuristic).
   *
  ...
  /**
   * getter.
   * @return the nodes evaluated in the run
   */
  ...
  double calculateHeuristicValue(State<Point> *element_to_calculate) {
    double val;
    val = abs(this->goal_state->getState().first
                  - element_to_calculate->getState().first);
    val += abs(this->goal_state->getState().second
                   - element_to_calculate->getState().second);
    return val;
  }
};
